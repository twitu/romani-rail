...
WITH evening_trains AS (
  SELECT ts.train_no, tr.train_name, tr.destination_station, ts.departure_time AS start_time, ts.seq AS start_seq, ts.station_code AS start_station, ts.distance as start_distance
...
morning_stations AS (
  SELECT ts.train_no, ts.station_code, ts.arrival_time AS end_time, ts.seq AS end_seq, ts.distance AS end_distance
  FROM train_schedule ts
...
trains_with_travel_time AS (
  ...
  JOIN evening_trains et ON ms.train_no = et.train_no AND ms.end_seq > et.start_seq AND ms.end_distance - et.start_distance < 1000
),
final_result AS (
